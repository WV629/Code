...
class Spider(object):
    ...
    def thread_parse(self,div):
        ...
        list1['标题'] = div.xpath(".//header[@class='post-header']/h1/text()")[0].strip()
        try:
            list1['日期'] = detail_html.xpath("//span[@class='article-date']/text()")[0].strip().split(" ")[0].split("/")
            list1['日期'] = list1['日期'][2] + '-' + list1['日期'][1].zfill(2) + '-' + list1['日期'][0].zfill(2)
        except:
            list1['日期'] = ''
        try:
